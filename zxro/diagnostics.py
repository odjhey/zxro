...
def _check_owner_mode(path: Path, *, directory: bool) -> None:
    ...
    if not directory and info.st_size > MAX_LOG_FILE_BYTES:
        raise ValidationError("log file exceeds the size limit")
    ...
    def append(self, line: bytes) -> None:
        ...
        def write():
            ...
            try:
                ...
                try:
                    info = os.fstat(fd)
                    if not stat.S_ISREG(info.st_mode) or info.st_uid != os.geteuid() or stat.S_IMODE(info.st_mode) != 0o600 or info.st_nlink != 1 or info.st_size > MAX_LOG_FILE_BYTES:
