...
Upgraded built-in homes require authoritative metadata. If an M1 artifact has no metadata, routine reads fail under unsafe-state exit 5 with a migration-required message. The operator runs `zxro migrate artifact-metadata` once for all artifacts in the active home. Migration takes the store's mutation lock, fully validates each legacy artifact, preserves its bytes and reference, and publishes metadata with a true no-clobber create. If another writer wins creation, migration re-reads and compares that record instead of replacing it. Migration rejects conflicts and converges after interruption. Success reports migrated, already-indexed, and failed counts. An unsafe-state failure keeps normal JSON stdout empty and includes deterministic partial counts plus sorted affected references in the exit-5 diagnostic. Migration does not run during routine reads. Mailbox mutations create only the directories required for their own contracted records; pending, ack, and handle do not initialize artifact metadata or unrelated layout.
