...
| Large evidence | Per-turn files referenced from bounded JSON/JSONL records | Keeps routine watchtower context independent from accumulated report and log size | — |
...
- zxro-specific full-text search or log slicing. v0.x resolves artifact paths and relies on ordinary Unix readers when deeper evidence is needed.
...
- Routine read commands must not inline large artifact bodies. Inbox events and summaries remain bounded and refer to larger evidence by path-safe artifact reference.
- Filesystem mutations and artifact resolution must reject unsafe paths, symlinks where ownership matters, malformed JSON, and conflicting state rather than guessing.
