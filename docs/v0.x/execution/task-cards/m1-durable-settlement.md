...
| Direct identity generation uses one strict integer schema in consumers, partial-publication repair, and current-boundary settlement preflight, while ack rejects non-integer API values before state access | `DurableLoopCliTests.test_direct_index_generation_requires_strict_integer_for_all_consumers`, `test_partial_publication_index_corruption_precedes_unrelated_turn_mutation`, `test_published_boundary_index_corruption_precedes_requested_turn_mutation`, and `M1ProviderConformance.test_ack_api_rejects_non_integer_without_mutation` |
