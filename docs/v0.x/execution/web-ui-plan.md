...
- Proposed global contract:
  ...
  Validate logging flags and environment values before provider access. Correlation IDs are bounded opaque strings with a closed character set. Sensitive mode emits a raw resource correlation only after the matching ZXRO ID or reference validator accepts it; malformed, path-like, and oversized arguments are omitted or keyed-fingerprinted before emission. A log file is explicit, must resolve outside the active `$ZXRO_HOME`, and must pass owner, permission, file-type, link-count, parent-directory, and symlink checks. File-backed logs use the fixed retention defaults in this plan unless a later contract adds bounded retention flags.
...
- Output and exit behavior: with logging off, successful stderr remains empty and failures keep the current bounded human diagnostic. With `--log-format jsonl` to stderr, every stderr line follows the log schema. Every invocation with a level other than `off` constructs exactly one terminal `zxro.cli.invocation.completed` event, even when its success level would otherwise fall below the threshold. The terminal event is the final and highest-sequence event and carries `process_exit_code` plus overall `result_code` or `error_code`. A healthy enabled sink receives it exactly once. Stage events never carry `process_exit_code`; they use `result_code` on success or stable `error_code` on failure. A stage completes only after its full observed operation returns, and a stage failure names only a boundary that execution entered. With `--log-file`, normal command stderr keeps its current behavior while structured events go to the file. Invalid logging configuration exits 2 before state access. A runtime sink, append, rotation, formatting, or redaction failure disables that sink without a fallback warning (this single-sink CLI has no remaining destination to report through), and never changes the underlying command exit code, retries a mutation, or redirects logs to stdout or `$ZXRO_HOME`. See the shipped contract in [Structured CLI logging](../engineering/structured-cli-logging.md).
